...
class UnPairedDualDataset(data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        if self.opt["color"]:
            ...
            if self.opt["phase"] == "train":
                img_src_1, img_src_2, img_tgt = util.channel_convert(self.opt["color"], [img_src_1, img_src_2, img_tgt])
            else:
        ...
        if img_src.shape[2] == 3:
            if self.opt["phase"] == "train":
                img_src_1 = img_src_1[:, :, [2, 1, 0]]
                img_src_2 = img_src_2[:, :, [2, 1, 0]]
        ...
        if self.opt["phase"] == "train":
            img_src = np.array([img_src_1, img_src_2])
            img_src = torch.from_numpy(
                np.ascontiguousarray(np.transpose(img_src, (0, 3, 1, 2)))
            ).float()
        else:
            img_src = torch.from_numpy(
