...
def validate(model, dataset, dist_loader, opt, measure, epoch, current_step):
    ...
    for (
        ...
        LR_img = val_data["src"]
        lr_img = util.tensor2img(LR_img, min_max=(0, 255))  # save LR image for reference
        ...
        ### TODO: press codes to save the images as you want
        ...
        if "tgt" in val_data.keys():
            gt_img = util.tensor2img(val_data["tgt"], min_max=(0, 255))
            save_gt_path = os.path.join(img_dir, "{:s}_GT.png".format(img_name))
